...
#define AlertViewWidth 320.0f
#define AlertViewHeight 181.0f
...
-(UIImage*)buttonImageByStyle:(XYButtonStyle)style state:(UIControlState)state
{
    switch(style)
    {
        ...
        case XYButtonStyleGray:
            return [[UIImage imageNamed:@"alertView_button_cancel"] stretchableImageWithLeftCapWidth:22 topCapHeight:22];
        case XYButtonStyleGreen:
            return [[UIImage imageNamed:@"alertView_button_confirm"] stretchableImageWithLeftCapWidth:22 topCapHeight:22];
    }
}
...
-(void)prepareAlertToDisplay:(XYAlertView*)entity
{
    ...
    UILabel *_titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(40, 35, 240, 30)];
    ...
    UILabel *_messageLabel = [[UILabel alloc] initWithFrame:CGRectMake(55, 33, 220, 60)];
    ...
    float buttonPadding = 100.0f / (entity.buttons.count - 1 + 2 * 2)+0.5;
    ...
    for(int i = 0; i < entity.buttons.count; i++)
    {
        ...
        _button.frame = CGRectMake(buttonPadding * 2 + buttonWidth * i + buttonPadding * i, 122,
                                   buttonWidth, 26);
        _button.tag = i;
