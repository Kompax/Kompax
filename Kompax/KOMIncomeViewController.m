...
@interface KOMIncomeViewController ()

@end

@implementation KOMIncomeViewController
...
- (void)viewDidLoad
{
    ...
    _cash.textColor = [UIColor colorWithRed:178/255.0 green:201/255.0 blue:128/255.0 alpha:1.0];
...
-(void)calculateAndDraw {
    ...
    draw.mode = kEarnMode;      //当前画图为收入画图模式
    
    if (didEarn + current <= planedEarn) {
        
        draw.drawingMode = kNotOverearned;
        draw.larger = planedEarn;
        draw.smaller = didEarn + current;
        
    }   //未超支
    else {
        draw.drawingMode = kOverearned;
        draw.smaller = planedEarn;
        draw.larger = didEarn + current;
    }   //已超支
...
-(void)presentCategory {
    ...
    KOMNavViewController * nav = [self.storyboard instantiateViewControllerWithIdentifier:@"AccNav"];
    KOMIncomeCategoryTableViewController *categoryVC = [self.storyboard instantiateViewControllerWithIdentifier:@"IncomeCategory"];
...
#pragma mark - KOMInomceCategoryTableViewController delegate
